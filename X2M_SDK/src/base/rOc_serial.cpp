...
char rOc_serial::readChar(unsigned char *pByte, unsigned int TimeOut_ms)
{
#if defined (_WIN32) || defined(_WIN64)
    if (hSerial == INVALID_HANDLE_VALUE)
    {
    ...
    TimeOut         Timer;                                              // Timer used for timeout
    Timer.InitTimer();                                                  // Initialise the timer
    while (Timer.Duation_ms() < TimeOut_ms || TimeOut_ms == 0)          // While Timeout is not reached
    {
        switch (read(fd, pByte, 1))
        {                                     // Try to read a byte on the device
        case 1: return 1;                                             // Read successfull
...
/*!
\brief Read an array of bytes from the serial device (with timeout). Stable!
*/
int rOc_serial::readChars(unsigned char *Buffer, unsigned int MaxNbBytes, unsigned int TimeOut_ms)
{
    ...
    char            ret = 0;                                                // Returned value from Read
    TimeOut         Timer;                                              // Timer used for timeout
    ...
    while (NbBytes < MaxNbBytes)                                          // While the buffer is not full
    {
        ...
        if (TimeOutParam > 0)                                             // If the parameter is higher than zero
        {
            ...
            ret = readChar(&curByte, 60);                // Wait for a byte on the serial link
            if (1 == ret)                                                 // If a byte has been read
            {
                Buffer[NbBytes] = curByte;
                NbBytes++;
        ...
        {
            Buffer[NbBytes] = 0;                                          // Add the end caracter
            return 0;                                                   // Return 0
        }
    }
